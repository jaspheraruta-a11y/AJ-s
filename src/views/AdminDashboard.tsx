...
const REPORT_BRAND = {
  legalName: "AJ's Café",
  tagline: 'Orders & sales report',
  addressLines: ['Laligan, Valencia, Bukidnon', 'Mindanao, Philippines'],
  /** Set to your public phone / email for printed reports */
  phone: '' as string,
  email: '' as string,
  accent: '#5c4d4f',
  accentSoft: '#f5f0ef',
...
function escapeHtmlPdf(value: string | number | null | undefined): string {
...
function formatOrderTypeLabel(t: string | undefined): string {
  const x = (t || '').toLowerCase();
  if (x === 'walkin' || x === 'walk-in') return 'Walk-in';
  if (x === 'online') return 'Online';
  if (x === 'qr') return 'QR';
  return escapeHtmlPdf(t || '—');
}
...
const exportOrdersToPDF = async (orders: Order[], customers: Profile[], generatedBy: Profile) => {
  ...
  const totalSalesCompleted = completed.reduce((sum, o) => sum + Number(o.total || 0), 0);
  const completedCount = completed.length;
  const avgOrderValue = completedCount > 0 ? totalSalesCompleted / completedCount : 0;
  const cancellationRate = totalOrders > 0 ? (cancelled.length / totalOrders) * 100 : 0;
  const sumAllTotals = orders.reduce((sum, o) => sum + Number(o.total || 0), 0);

  const generatedByName = escapeHtmlPdf(generatedBy.full_name || generatedBy.email || 'System');
  const generatedByEmail = escapeHtmlPdf(generatedBy.email || '');

  const verificationPayload = [
    `${REPORT_BRAND.legalName} — Orders Report`,
    `Report ID: ${reportId}`,
    `Generated: ${generatedAt.toISOString()}`,
    `Generated by: ${generatedBy.full_name || generatedBy.email}`,
    `Rows: ${totalOrders}`,
  ].join('\n');
  ...
  const rows = orders
    .map((order) => {
      ...
      const customer = escapeHtmlPdf(cust?.full_name || 'Walk-in Customer');
...
<head>
  ...
  <title>${escapeHtmlPdf(REPORT_BRAND.legalName)} — Orders Report</title>
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    header.report-header {
      ...
      padding-bottom: 16px;
      margin-bottom: 18px;
      border-bottom: 3px double ${REPORT_BRAND.accent};
    }
    .brand-mark {
      width: 52px;
      height: 52px;
    ...
    .meta-pill .label { color: #78716c; text-transform: uppercase; letter-spacing: 0.06em; font-size: 9px; }
    .meta-pill .value { font-weight: 700; color: #1c1917; margin-top: 2px; }
    ...
    .exec-card .sub { font-size: 10px; color: #78716c; margin-top: 4px; font-weight: 500; }
    ...
    .section-title {
      ...
      margin: 0 0 8px;
      padding-bottom: 6px;
      border-bottom: 2px solid ${REPORT_BRAND.accentSoft};
    }
    ...
    table.data thead th {
      ...
      border: 1px solid ${REPORT_BRAND.accent};
    }
    ...
    table.data tbody tr:nth-child(even) td { background: #fafaf9; }
    table.data tbody tr:nth-child(odd) td { background: #fff; }
    table.data td.num { text-align: right; font-variant-numeric: tabular-nums; }
    table.data td.mono { font-family: ui-monospace, 'Cascadia Code', Consolas, monospace; }
    ...
    .status-completed { background: #dcfce7; color: #14532d; border: 1px solid #86efac; }
    .status-cancelled { background: #fee2e2; color: #991b1b; border: 1px solid #fca5a5; }
    .status-progress { background: #fef3c7; color: #92400e; border: 1px solid #fcd34d; }

    table.data tfoot td {
      padding: 10px;
      border: 1px solid #d6d3d1;
      background: ${REPORT_BRAND.accentSoft};
      font-weight: 700;
      font-size: 10.5px;
    }
    ...
    .audit-left { font-size: 10px; color: #44403c; line-height: 1.6; }
    .audit-left .row { margin-bottom: 6px; }
    .audit-left .k { color: #78716c; font-weight: 600; display: inline-block; min-width: 100px; }
    .signature-block {
      margin-top: 14px;
      padding-top: 8px;
    }
    ...
    .sig-caption { font-size: 9px; color: #78716c; }
    .qr-box {
      border: 1px solid #d6d3d1;
      border-radius: 8px;
      padding: 10px;
    ...
    .qr-box img { display: block; margin: 0 auto 8px; width: 120px; height: 120px; }
    .qr-box .cap { font-size: 8px; color: #57534e; line-height: 1.35; }
    @media print {
  ...
  <div class="doc">
    <header class="report-header">
      <div style="display:flex;gap:14px;align-items:flex-start;">
        <div class="brand-mark" aria-hidden="true">AJ</div>
        <div class="brand-block">
          <h1>${escapeHtmlPdf(REPORT_BRAND.legalName)}</h1>
          <p class="doc-title">${escapeHtmlPdf(REPORT_BRAND.tagline)}</p>
          <div class="contact">
            <strong>Business address</strong><br />
            ${REPORT_BRAND.addressLines.map((l) => escapeHtmlPdf(l)).join('<br />')}
            ${REPORT_BRAND.phone ? `<br /><strong>Phone</strong> ${escapeHtmlPdf(REPORT_BRAND.phone)}` : ''}
            ${REPORT_BRAND.email ? `<br /><strong>Email</strong> ${escapeHtmlPdf(REPORT_BRAND.email)}` : ''}
          </div>
        </div>
      </div>
      <div class="meta-pill">
        <div class="label">Report ID</div>
        <div class="value mono" style="font-size:10px;word-break:break-all;">${escapeHtmlPdf(reportId)}</div>
        <div style="margin-top:12px;" class="label">Generated</div>
        <div class="value" style="font-weight:600;font-size:10px;">${escapeHtmlPdf(generatedAtStr)}</div>
      </div>
    </header>
    ...
    <div class="exec-summary">
      <div class="exec-card">
        <div class="label">Total sales (completed only)</div>
        <div class="value">&#8369;${totalSalesCompleted.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        <div class="sub">${completedCount} completed order${completedCount === 1 ? '' : 's'}</div>
      </div>
      <div class="exec-card">
        <div class="label">Average order value</div>
        <div class="value">&#8369;${avgOrderValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        <div class="sub">Based on completed orders</div>
      </div>
      <div class="exec-card">
        <div class="label">Cancellation rate</div>
        <div class="value">${cancellationRate.toFixed(1)}%</div>
        <div class="sub">${cancelled.length} cancelled of ${totalOrders} total</div>
      </div>
      <div class="exec-card">
        <div class="label">Total orders</div>
        <div class="value">${totalOrders}</div>
        <div class="sub">All statuses in this export</div>
      </div>
    </div>
    ...
    <div class="audit">
      <div class="audit-left">
        <div class="row"><span class="k">Report ID</span> <span class="mono">${escapeHtmlPdf(reportId)}</span></div>
        <div class="row"><span class="k">Generated by</span> ${generatedByName}${generatedByEmail ? ` <span style="color:#78716c;">(${generatedByEmail})</span>` : ''}</div>
        <div class="row"><span class="k">Role</span> ${escapeHtmlPdf(generatedBy.role)}</div>
        <div class="row"><span class="k">Generated at</span> ${escapeHtmlPdf(generatedAtStr)}</div>
        <div class="signature-block">
          <div class="signature-line"></div>
          <div class="sig-caption">Authorized signature &amp; date (digital / wet signature for internal records)</div>
        </div>
      </div>
      <div class="qr-box">
        <img src="${qrSrc}" width="120" height="120" alt="Verification QR" />
        <div class="cap">Scan to verify report metadata. Retain this page for audit trail.</div>
      </div>
    </div>
  </div>
</body>
...
function summarizeAdminActivity(log: AdminLog): string {
  const d = (log.details || {}) as Record<string, unknown>;
  switch (log.action) {
    case 'update_order_status':
      return `Changed an order's status to "${String(d.status ?? '—')}".`;
    case 'update_product_availability':
      return `Set product availability to ${d.is_available ? 'available' : 'unavailable'}.`;
    case 'update_inventory':
      return `Updated inventory stock to ${String(d.quantity ?? '—')}.`;
    case 'create_promo':
      return `Created promo "${String(d.product_name ?? 'new promo')}".`;
    default:
      return log.action.replace(/_/g, ' ');
  }
}

function formatLoginMethod(method: string | null | undefined): string {
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout, mode = 'admin' }) => {
  ...
  useEffect(() => {
    setInventoryPage(1);
  }, [inventorySearchTerm, inventoryCategoryFilter, inventoryFilters.lowStock, inventoryFilters.optimal]);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-t border-stone-100 bg-stone-50/60">
          <p className="text-sm text-stone-500">
            {filteredOrders.length === 0
              ? 'No orders to show'
              : `Showing ${(ordersPage - 1) * TABLE_PAGE_SIZE + 1}–${Math.min(ordersPage * TABLE_PAGE_SIZE, filteredOrders.length)} of ${filteredOrders.length}`}
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setOrdersPage(p => Math.max(1, p - 1))}
              disabled={ordersPage <= 1 || filteredOrders.length === 0}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl border border-stone-200 bg-white text-stone-700 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <button
              ...
              disabled={ordersPage >= ordersTotalPages || filteredOrders.length === 0}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl border border-stone-200 bg-white text-stone-700 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Next
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-6 py-4 border-t border-stone-100 bg-stone-50/60">
          <p className="text-sm text-stone-500">
            {filteredInventory.length === 0
              ? 'No inventory rows to show'
              : `Showing ${(inventoryPage - 1) * TABLE_PAGE_SIZE + 1}–${Math.min(inventoryPage * TABLE_PAGE_SIZE, filteredInventory.length)} of ${filteredInventory.length}`}
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() => setInventoryPage(p => Math.max(1, p - 1))}
              disabled={inventoryPage <= 1 || filteredInventory.length === 0}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl border border-stone-200 bg-white text-stone-700 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              <ChevronLeft className="w-4 h-4" />
              Back
            ...
            <button
              ...
              disabled={inventoryPage >= inventoryTotalPages || filteredInventory.length === 0}
              className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl border border-stone-200 bg-white text-stone-700 hover:bg-stone-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Next
              <ChevronRight className="w-4 h-4" />
  ...
  const resolveProfileLabel = (userId: string | null | undefined) => {
    if (!userId) return { name: 'System', sub: '' };
    const p = users.find(u => u.id === userId);
    if (!p) return { name: 'Unknown user', sub: userId.slice(0, 8) + '…' };
    const roleLabel = p.role === 'admin' ? 'Admin' : p.role === 'staff' ? 'Staff' : p.role;
    return { name: p.full_name || p.email || 'User', sub: roleLabel };
  };

  const recentActivitiesTab = (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-stone-800">Recent activities</h2>
        ...
        {filteredActivities.length === 0 ? (
          ...
          <ul className="divide-y divide-stone-100">
            {pagedActivities.map((log) => {
              ...
              return (
                <li key={log.id} className="p-5 hover:bg-stone-50/80 transition-colors">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                    <div className="flex gap-4 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-[#f5f0ef] border border-stone-100 flex items-center justify-center shrink-0">
                        <History className="w-5 h-5 text-[#7b6a6c]" />
                      </div>
                      <div className="min-w-0">
                        <p className="font-semibold text-stone-800">{summarizeAdminActivity(log)}</p>
                        <p className="text-xs text-stone-500 mt-1">
                          <span className="font-medium text-stone-600">{actor.name}</span>
                          {actor.sub && (
                            <span className="text-stone-400"> · {actor.sub}</span>
                          )}
                          {log.entity_type && (
                    ...
                    <time className="text-xs text-stone-400 whitespace-nowrap sm:text-right shrink-0 font-mono">
                      {when.toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}
                      <br />
                      {when.toLocaleTimeString('en-PH', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                    </time>
                  </div>
                </li>
              );
  ...
  const loginLogsTab = (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-stone-800">Login logs</h2>
        <p className="text-sm text-stone-500 mt-1">
        ...
        {filteredLoginLogs.length === 0 ? (
          <div className="p-12 text-center space-y-2">
            ...
            <p className="text-xs text-stone-400 max-w-md mx-auto">
              After you run <code className="font-mono text-stone-600 bg-stone-100 px-1 rounded">staff_login_logs.sql</code> in Supabase, new staff and admin sign-ins from the shop login screen will appear here.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-stone-50 border-b border-stone-100 text-left text-xs font-bold uppercase tracking-wider text-stone-500">
                    ...
                    <tr key={row.id} className="hover:bg-stone-50/80 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="font-medium text-stone-800 truncate">{p?.full_name || p?.email || 'Unknown'}</p>
                            {p?.email && p?.full_name && (
                      ...
                      <td className="px-5 py-3 text-stone-600">{formatLoginMethod(row.method)}</td>
                      <td className="px-5 py-3 text-right text-stone-500 font-mono text-xs whitespace-nowrap">
